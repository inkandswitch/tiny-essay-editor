...
export const docTypes: Record<string, DataType<unknown>> = {
  essay: EssayDatatype,
...
export type HasDocMetadata<T> = Discussable<T>;

export type DocEditorProps<T> = {
  docUrl: AutomergeUrl;

  selectedDocAnchors: T[];
  hoveredDocAnchors: T[];
  setSelectedDocAnchors: (anchors: T[]) => void;
  setHoveredDocAnchors: (anchors: T[]) => void;
};
