...
const markCopy = (doc: AmbSheetDoc) => {
  doc.title = "Copy of " + doc.title;
};

const getTitle = async (doc: AmbSheetDoc) => {
  return doc.title || "Mystery Data Grid";
};

const setTitle = async (doc: AmbSheetDoc, title: string) => {
  doc.title = title;
};
...
const includeChangeInHistory =
  (doc: AmbSheetDoc) => (decodedChange: DecodedChangeWithMetadata) => {
    ...
    const rowObjIds = doc.data.map((_, index) =>
      A.getObjectId(doc.data, index)
    );
...
const includePatchInChangeGroup = (patch: A.Patch) =>
